...
public class UserService {
  ...
  public AuthResponse updateUser(
    ...
    if (!passwordEncoder.matches(request.getCurrentPassword(), currentUser.getPassword())) {
      throw new GlobalException("Current password is incorrect", "CONFLICT");
    }

    if (!applyUpdates(currentUser, request)) {
      throw new GlobalException("No fields to update", "CONFLICT");
    }
  ...
  public UserRelationsResponse getUserRelations(Long userId, Principal principal) {
    ...
    if (currentUserId.equals(userId)) {
      throw new GlobalException("Relations can't be found between two equal users", "CONFLICT");
    }
  ...
  private String authenticateAndGenerateToken(String username, String password) {
    try {
      ...
      throw new GlobalException("Invalid credentials", "CONFLICT");
    }
  }
