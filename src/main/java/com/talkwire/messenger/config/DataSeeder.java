...
public class DataSeeder {
    ...
    CommandLineRunner initDatabase(
        ...
        return args -> {
            ...
            if (userCount > 0) {
                logger.info("Database is not empty ({} users found). Skipping seed data creation.", userCount);
                return;
            }
                    ...
                    if (contactUser != null) {
                        ...
                        if (!createdContacts.contains(contactKey) && !createdContacts.contains(reverseKey)) {
                            try {
                                ...
                                contactsCreated++;
                                logger.info("Created contact: {} -> {}", user.getUsername(), contactUser.getUsername());
                            } catch (Exception e) {
                                logger.error("Failed to create contact: {} -> {}", user.getUsername(), contactUser.getUsername(), e);
                            }
                        }
                    }
                }
                    ...
                    if (contactUser != null) {
                        ...
                        if (!createdRequests.contains(requestKey) && !createdRequests.contains(reverseKey)
                            ...
                            try {
                                ...
                                logger.info("Created outgoing request: {} -> {}", user.getUsername(), contactUser.getUsername());
                            } catch (Exception e) {
                                logger.error("Failed to create outgoing request: {} -> {}", user.getUsername(), contactUser.getUsername(), e);
                            }
                        }
                    }
                }
                logger.info("Created {} outgoing requests for user: {}", outgoingRequestsCreated, user.getUsername());
                    ...
                    if (requestUser != null) {
                        ...
                        if (!createdRequests.contains(requestKey) && !createdRequests.contains(reverseKey)
                            ...
                            try {
                                ...
                                logger.info("Created incoming request: {} -> {}", requestUser.getUsername(), user.getUsername());
                            } catch (Exception e) {
                                logger.error("Failed to create incoming request: {} -> {}", requestUser.getUsername(), user.getUsername(), e);
                            }
                        }
                    }
                }
                logger.info("Created {} incoming requests for user: {}", incomingRequestsCreated, user.getUsername());
            }
